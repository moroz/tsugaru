...
func TestEd25119KeyService(t *testing.T) {
	ks, err := services.RS256KeyServiceFromPEM([]byte(privKey))
	require.NoError(t, err)

	t.Run("generates JWKS", func(t *testing.T) {
		jwks := ks.JWKS()
		jwk := jwks.Keys[0]
		assert.Equal(t, "2xikWpLFYFhsz67BynE8pH-FltC6zChHvnOE73tzcFM", jwk.KeyID)
		assert.Equal(t, "RS256", string(jwk.Algorithm))
		assert.Equal(t, "RSA", string(jwk.KeyType))
	})
}
